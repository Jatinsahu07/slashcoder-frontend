...
export default function DashboardStats({ userId }) {
  ...
  if (loading) {
    return <div className="p-4 rounded-xl bg-neutral-900/40">Loading your stats…</div>;
  }

  if (!you) return null;

  // ✅ use XP instead of totalPoints
  const xp = you.xp || 0;
  const { tier, emoji, progress } = getBadgeProgress(xp);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="text-3xl font-semibold text-white">
            #{rank}
            {totalUsers && (
              <span className="text-neutral-400 text-base"> / {totalUsers}</span>
            )}
          </div>
        </div>
        ...
        <div className="text-right">
          ...
          <div className="text-3xl font-semibold text-white">{xp}</div>
        </div>
      </div>
